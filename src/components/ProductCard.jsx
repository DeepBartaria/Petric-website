...
import { buildProductProperties, trackCleverTapEvent } from '../helper/clevertap';
...
export default function ProductCard({
  ...
  const handleAddClick = (event) => {
    ...
    trackCleverTapEvent('Add To Cart Clicked', buildProductProperties(product, {
      Source: trackingContext.Source || 'Product Card',
      ...trackingContext,
    }));

    if (hasMultipleVariants) {
      trackCleverTapEvent('Variant Selector Opened', buildProductProperties(product, {
        Source: trackingContext.Source || 'Product Card',
        ...trackingContext,
      }));
  ...
  return (
    <article
      onClick={() => {
        trackCleverTapEvent('Product Clicked', buildProductProperties(product, {
          Source: trackingContext.Source || 'Product Card',
          ...trackingContext,
        }));
    ...
    >
      <div className="relative flex h-[140px] w-full items-center justify-center overflow-hidden bg-gray-50 p-3 md:h-[160px]">
        <img
          ref={imageRef}
          src={product.img}
          alt={product.name}
          className="h-[85%] w-[85%] max-w-none object-contain mix-blend-multiply transition-transform duration-300 group-hover:scale-110"
        />

        {discount && discount !== '0%' && (
          // <div 
          //   className="absolute left-3 top-0 w-[38px] bg-[#00A650] flex flex-col items-center justify-start pt-1.5 pb-2.5 z-10"
          //   style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 83.33% 88%, 66.66% 100%, 50% 88%, 33.33% 100%, 16.66% 88%, 0 100%)' }}
          // >
          //   <span className="text-white text-[13px] font-black leading-none tracking-tight">{discount}</span>
          //   <span className="text-white text-[9px] font-extrabold leading-none mt-0.5 tracking-wider">OFF</span>
        ...
        {couponText && (
          // <div className="absolute right-1.5 top-1.5 inline-flex max-w-[78%] items-center gap-1 rounded-lg border-[1.5px] border-dashed border-[#00A650] bg-[#E8F8EE] px-2 py-1 text-[10px] font-extrabold leading-tight text-[#00A650] shadow-sm">
        ...
        */}
        ...
        {totalCartQuantity > 0 ? (
          ...
          <motion.button
            ...
            className="relative overflow-hidden mt-auto h-10 w-full rounded-lg bg-[#FFD000] text-[13px] font-black text-black shadow-sm transition-colors hover:bg-[#ffdb33]"
          >
