  ...
  const handleUseMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        trackCleverTapEvent('Permission Granted', {
          'Permission Type': 'Location',
          Source: 'Delivery Location Modal',
        });
        trackCleverTapEvent('Location Selected', {
      ...
      (error) => {
        setIsFetchingGPS(false);
        trackCleverTapEvent('Permission Denied', {
          'Permission Type': 'Location',
          Source: 'Delivery Location Modal',
        });
  ...
  const handleCalculateTime = () => {
    ...
    service.getDistanceMatrix(
      ...
      (response, status) => {
        ...
        const addressText = deliveryLocation.address || 'Address not available';
        logActivity(
          `User Entered Location: ${addressText}`,
          'Web_UserLocation'
        );
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-2">
                ...
                <button
                  ...
                  className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-2 bg-black hover:bg-gray-800 disabled:bg-gray-300 text-white px-4 py-3 rounded-xl font-semibold text-sm transition-all shadow-sm whitespace-nowrap"
                >
                  {isFetchingGPS ? (
                  ...
                  <span>{isFetchingGPS ? 'Locating...' : 'Use my location'}</span>
                </button>
              </div>
              ...
              {deliveryTime && (
                <div className="flex items-center justify-center gap-2 text-green-700 font-bold bg-green-50 px-4 py-3 rounded-xl border border-green-200 text-sm">
                  ...
                  Estimated delivery: {String(deliveryTime).includes('By') ? deliveryTime : `${deliveryTime} minutes`} 🎉
                </div>
              )}
            </>
          ) : (
