...
export default function NewHomeNavbar() {
  ...
  const navigateToProduct = (product) => {
    trackCleverTapEvent('Search Result Clicked', {
  ...
  const navigateToSearch = (query) => {
    trackCleverTapEvent('Search Performed', {
      'Search Query': query,
      'Search Type': 'Text',
    ...
    <>
      ...
      <div className="bg-white flex flex-col md:flex-row md:items-center justify-between w-full">
        ...
        <div className="flex w-full md:w-auto flex-1 items-center gap-3 px-4 py-1.5 md:py-0 md:px-4 lg:px-8 border-t border-gray-100 md:border-0 order-last md:order-none">
          {/* Hamburger (Mobile) */}
          <div className="lg:hidden relative shrink-0 z-50">
            ...
            {isMobileMenuOpen && (
              <div className="absolute top-10 left-0 w-48 bg-white shadow-xl rounded-xl border border-gray-100 flex flex-col py-2">
                <button
                  onClick={() => {
                    setIsMobileMenuOpen(false);
                    window.open('https://petric.in/download/', '_blank');
                    logActivity('User Click Download App button', 'Web_DownloadApp');
                  }}
                ...
                {!user && (
                  <button 
                    onClick={() => { setIsMobileMenuOpen(false); window.dispatchEvent(new CustomEvent('openCart', { detail: { step: 'mobile' } })); }}
                ...
                {user && (
                  <>
                    ...
                    <button
                      onClick={() => { setIsMobileMenuOpen(false); setIsOrdersOpen(true); }}
                      className="w-full text-left px-4 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 flex items-center gap-2 border-t border-gray-100"
                    >
                      Your Orders
                    </button>
                    <button
                      onClick={() => { setIsMobileMenuOpen(false); navigate('/saved-addresses'); }}
                      className="w-full text-left px-4 py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 flex items-center gap-2 border-t border-gray-100"
                    >
                      Saved Address
                    </button>
                    <button
                      onClick={() => { localStorage.removeItem('petric_user'); localStorage.removeItem('petric_token'); setUser(null); window.location.reload(); }}
        ...
        <div className="lg:hidden flex items-center shrink-0 relative">
          <Link
            to="/reorder"
            onClick={() => logActivity('User Click Re-order Upper Bar', 'Web_Reorder')}
            className="flex items-center gap-1.5 bg-[#FFD000] text-black px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap shadow-sm active:scale-95 transition-transform"
          >
            ...
            {user && (
              <div className="absolute right-0 top-full pt-2 w-32 hidden group-hover:block z-50">
                <div className="bg-white rounded-xl shadow-xl border border-gray-100 py-2">
                  <button
                    onClick={() => {
                      trackCleverTapEvent('Profile Viewed', { Section: 'User Profile', Source: 'Navbar' });
                  ...
                  <button
                    onClick={() => {
                      trackCleverTapEvent('Past Orders Viewed', { Source: 'Navbar' });
                  ...
                  <button
                    onClick={() => {
                      trackCleverTapEvent('Saved Addresses Viewed', { Source: 'Navbar' });
        ...
        <div className="md:hidden flex items-center relative">
          ...
          <div className="shrink-0 z-10">
            <Link
              to="/all-categories"
              onClick={() => {
                window.dispatchEvent(new CustomEvent('resetCategories'));
                trackCleverTapEvent('Navigation Clicked', {
                  'Navigation Item': 'All Categories',
            ...
            <div className="flex items-center gap-1 overflow-x-auto [&::-webkit-scrollbar]:hidden px-1">
              {navCategories.map((category) => (
                <Link
                  ...
                  onClick={() => trackCleverTapEvent('Category Clicked', {
                  ...
                  {hoveredCategory?._id === category._id && categorySubcategories[category._id]?.length > 0 && (
                    ...
                    >
                      {categorySubcategories[category._id].map((sub) => (
                        <Link
                          key={sub._id}
                          to={`/category/${createSlug(category.name)}?subCategory=${createSlug(sub.name)}`}
