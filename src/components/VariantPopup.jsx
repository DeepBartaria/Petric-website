...
export default function VariantPopup({ isOpen, onClose, product, onAddToCart, onAnimateToCart }) {
  ...
  const handleAddVariant = (variant) => {
    ...
    trackCleverTapEvent('Variant Selected', buildProductProperties(variantProduct, {
      Source: 'Variant Popup',
    }));
    trackCleverTapEvent('Added To Cart', buildProductProperties(variantProduct, {
      Quantity: 1,
      'Item Value': variantProduct.discountedPrice || variantProduct.price,
      Source: 'Variant Popup',
    }));
  ...
  const handleMinusVariant = (variant) => {
    ...
    trackCleverTapEvent('Cart Quantity Updated', buildProductProperties(product, {
      'Variant ID': variant.id || variant._id,
      'Variant Name': variant.weight || variant.name,
      'Old Quantity': cartItem.quantity,
      'New Quantity': cartItem.quantity - 1,
      Source: 'Variant Popup',
    }));
                      ...
                      >
                        ...
                        {quantity > 0 ? (
                          ...
                          <motion.button
                            ...
                            className="relative overflow-hidden h-9 min-w-[82px] rounded-lg bg-[#FFD000] px-4 text-sm font-extrabold text-black transition-colors hover:bg-[#ffdb33]"
                          >
