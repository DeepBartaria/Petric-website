...
import { buildCleverTapUserProfile, buildProductProperties, trackCleverTapEvent, onUserLogin } from '../helper/clevertap';
import { logActivity } from '../helper/analytics';
import loaderGif from '../assets/loader.gif';
import awganf from '../assets/awganf.jpg';
import lmif from '../assets/lmif.jpg';
...
export default function CartSidebar({ isOpen, onClose, cartItems, onUpdateQuantity, onLoginSuccess, loginBackCloses = false }) {
  ...
  useEffect(() => {
    if (isOpen) {
      trackCleverTapEvent('Cart Viewed', getCartEventProperties({
        Source: 'Cart Sidebar',
      }));
      if (cartItems.length > 0) {
        trackCleverTapEvent('Checkout Started', getCartEventProperties({
          Source: 'Cart Sidebar',
        }));
      }
    }
  ...
  const getCartEventProperties = (extra = {}) => ({
    'Cart Value': totalPayable,
    'Cart Item Count': cartItems.reduce((total, item) => total + (Number(item.quantity) || 0), 0),
    'Unique Item Count': cartItems.length,
    'Coupon Code': appliedCoupon?.code,
    'Discount Amount': couponDiscount || undefined,
    ...extra,
  });
  ...
  const handleSaveCheckoutAddress = async (event) => {
    ...
    try {
      ...
      if (response?.type === 'success') {
        ...
        logActivity('User Added New Address in Cart', 'Web_AddAddress');
      } else {
  ...
  const handleSelectAddress = async (address) => {
    ...
    setCheckoutStep('cart');
    logActivity('User Selected Address in Cart', 'Web_SelectAddress');
  };
  ...
  useEffect(() => {
    ...
    const handleOpenCart = (e) => {
      ...
      logActivity('User Opened Cart Sidebar', 'Web_OpenCart');
    };
  ...
  useEffect(() => {
    if (isOpen && checkoutStep === 'cart' && availableCoupons.length === 0) {
      fetchCoupons();
    }
  }, [isOpen, checkoutStep]);
    ...
    try {
      ...
      if (res && res.type === 'error' && res.message === 'Account not found') {
        ...
        if (regRes && regRes.type === 'success') {
          trackCleverTapEvent('Signup Started', {
            Method: 'OTP',
            Source: loginOnlyMode ? 'Login Modal' : 'Checkout',
          });
          trackCleverTapEvent('OTP Requested', {
            Method: 'OTP',
            Source: loginOnlyMode ? 'Login Modal' : 'Checkout',
          });
          setCheckoutStep('otp');
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (res && res.type === 'success') {
        ...
        onUserLogin(buildCleverTapUserProfile({
          ...loggedInUser,
          email: res.email,
          udid: res.udid,
        }));

        const isNewUser = Boolean(res.message?.toLowerCase().includes('register') || res.isNewUser || res.isNew);
        trackCleverTapEvent(isNewUser ? 'Signup Completed' : 'Login Completed', {
          Method: 'OTP',
          Source: loginOnlyMode ? 'Login Modal' : 'Checkout',
          'New User': isNewUser,
        });
  ...
  const handlePayment = async () => {
    ...
    logActivity(`User Initiated Checkout for ₹${totalPayable}`, 'Web_ProceedToCheckout');
    ...
    if (paymentMethod === 'COD') {
      ...
      logActivity(`User Purchased Order ₹${totalPayable} via COD`, 'Web_Purchase');
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        logActivity(`User Purchased Order ₹${totalPayable} via Razorpay`, 'Web_Purchase');
    ...
    try {
      ...
      if (res && res.coupons) {
        ...
        if (found) {
          ...
          logActivity(`User Applied Coupon ${found.couponPromoCode}`, 'Web_ApplyCoupon');
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className="fixed top-0 right-0 h-[100dvh] w-full sm:w-[400px] bg-[#f8f9fa] z-[250] flex flex-col shadow-2xl transform transition-transform duration-300 translate-x-0 font-sans">
        ...
        <div 
          ...
          className={`flex-1 overflow-y-auto flex flex-col ${checkoutStep === 'mobile' || checkoutStep === 'otp' ? 'bg-[#FFD000]' : 'p-4 gap-4 pb-6'}`}
        >
          {checkoutStep === 'cart' && (
            <>
                        ...
                        <div className="flex h-6 items-center overflow-hidden rounded-md bg-[#FFD000] text-black shadow-sm shrink-0">
                          <button
                            ...
                            onClick={() => {
                              ...
                              trackCleverTapEvent(nextQuantity > 0 ? 'Cart Quantity Updated' : 'Item Removed From Cart', buildProductProperties(item, getCartEventProperties({
                          ...
                          <button
                            className="grid h-6 w-7 place-items-center hover:bg-black/10"
                            onClick={() => {
                              trackCleverTapEvent('Cart Quantity Updated', buildProductProperties(item, getCartEventProperties({
                                'Old Quantity': item.quantity,
                                'New Quantity': item.quantity + 1,
                                Source: 'Cart Sidebar',
                              })));
                              onUpdateQuantity(item.id, item.quantity + 1);
                            }}
              ...
              {/* Coupon Section */}
              <div className="w-full bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-3">
                <div 
                  className="flex items-center justify-between cursor-pointer" 
                  onClick={() => {
                    trackCleverTapEvent('Coupon List Viewed', getCartEventProperties({
                      'Available Coupon Count': availableCoupons.length || undefined,
                      Source: 'Cart Sidebar',
                    }));
                    fetchCoupons();
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-gray-900 text-[15px]">Apply coupon</span>
                  </div>
                  <button className="text-gray-500 hover:text-gray-800">
                    <FiChevronRight className="h-4 w-4 shrink-0 text-gray-600" />
                  </button>
                </div>

                {!isCouponsLoading && availableCoupons.length > 0 && (
                  <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-400">
                    {availableCoupons.map((c) => {
                      const isEligible = !c.couponMinimumAmount || itemsTotal >= c.couponMinimumAmount;

                      let desc = "";
                      if (c.couponType === "0") {
                        desc = `${c.couponPrice}% off`;
                      } else {
                        desc = `₹${c.couponPrice} off`;
                      }
                      if (c.couponMinimumAmount) desc += ` on orders above ₹${c.couponMinimumAmount}`;

                      const containerClass = isEligible 
                        ? "bg-[#F5FBF7] border-[#A8D7BA]" 
                        : "bg-white border-gray-200";
                      const titleClass = isEligible ? "text-[#2B7D4F]" : "text-gray-600";
                      const buttonClass = isEligible 
                        ? "bg-[#347B54] text-white hover:bg-[#2B7D4F] border border-[#347B54]" 
                        : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50";

                      const isApplied = appliedCoupon?.code === c.couponPromoCode;

                      return (
                        <div key={c._id} className={`w-[220px] rounded-xl border border-dashed p-3 flex flex-col justify-between shrink-0 ${containerClass}`}>
                          <div>
                            <div className="flex items-center justify-between mb-1.5">
                              <span className={`font-bold text-[15px] ${titleClass}`}>{c.couponPromoCode}</span>
                            </div>
                            <p className="text-[12px] text-gray-600 mb-3">{desc}</p>
                          </div>
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (isApplied) {
                                trackCleverTapEvent('Coupon Removed', getCartEventProperties({
                                  'Coupon Code': c.couponPromoCode,
                                  Source: 'Cart Sidebar',
                                }));
                                setAppliedCoupon(null);
                                setCouponCode('');
                                ...
                                trackCleverTapEvent('Coupon Applied', getCartEventProperties({
                                  'Coupon Code': c.couponPromoCode,
                                  Source: 'Cart Sidebar',
                                }));
                            ...
                            className={`w-full py-1.5 rounded-lg text-sm font-bold transition-colors ${buttonClass} ${!isEligible && !isApplied ? 'opacity-70 cursor-not-allowed' : ''}`}
                          >
          ...
          {checkoutStep === 'mobile' && (
            <div className="flex flex-col min-h-full w-full bg-[#FFD000]">
              <div className="relative w-full">
                <button 
                  onClick={handleBack} 
                  className="absolute top-4 left-4 p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm hover:bg-white transition-colors z-10"
                >
                  <FiArrowLeft className="h-5 w-5 text-black" strokeWidth={2.5} />
                </button>
                <img src={awganf} alt="Are we getting a new friend?" className="w-full object-cover" />
              </div>
              <div className="bg-white rounded-t-[32px] flex-1 px-6 pt-8 pb-4 flex flex-col items-center relative z-10 -mt-8">
                <h3 className="text-[18px] font-bold text-black mb-6">Enter Mobile Number</h3>
                
                <div className="flex items-center border border-black rounded-[32px] overflow-hidden w-full h-14">
                  <span className="px-4 text-black font-bold text-[16px] border-r border-black flex items-center h-[60%]">+91</span>
                  <input
                    type="tel"
                    inputMode="numeric"
                    ...
                    maxLength={10}
                    className="flex-1 px-4 text-[16px] text-black outline-none h-full placeholder-gray-400 font-medium"
                  />
                </div>
                ...
                <button
                  ...
                  className={`w-full mt-6 h-14 rounded-full flex items-center justify-center font-bold text-[18px] transition-all duration-300 ${
                    isMobileValid 
                      ? 'bg-[#FFD000] hover:bg-[#ffdb33] text-black shadow-sm transform hover:scale-[1.02]' 
                      : 'bg-[#FFD000] opacity-70 text-black cursor-not-allowed'
                  }`}
                >
                  Continue
                </button>

                <p className="mt-8 text-[12px] text-gray-500 text-center">
                  By continuing, you agree to our<br/>
                  <a href="/terms-and-conditions" className="underline hover:text-gray-800">Terms of service</a> | <a href="/privacy-policy" className="underline hover:text-gray-800">Privacy Policy</a>
                </p>

                <div className="mt-12 flex flex-col items-center">
                  <img src={logo} alt="Petric" className="h-10 mb-2 object-contain" />
                  <p className="text-[15px] font-medium text-black flex items-center gap-1">
                    Pet Care in Minutes! <span className="text-red-600">❤️</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          {checkoutStep === 'otp' && (
            <div className="flex flex-col min-h-full w-full bg-[#FFD000]">
              <div className="relative w-full">
                ...
                <img src={lmif} alt="Let's make it official" className="w-full object-cover" />
              </div>
              <div className="bg-white rounded-t-[32px] flex-1 px-6 pt-8 pb-4 flex flex-col items-center relative z-10 -mt-8">
                <h3 className="text-[20px] font-bold text-black mb-1">Enter Your OTP</h3>
                <p className="text-[14px] text-gray-500 mb-8 flex items-center justify-center text-center">
                  A 4-digit code has been sent to<br/>+91 {mobileNumber} 
                  <button onClick={() => setCheckoutStep('mobile')} className="text-black ml-1 hover:text-gray-700">
                    <FiEdit3 size={16} />
                  </button>
                </p>

                <div className="flex justify-center w-full max-w-[320px]">
                  <div className="relative flex gap-4 justify-between w-full">
                    {[0,1,2,3].map(i => (
                      <div key={i} className="w-[60px] h-[60px] rounded-[16px] border border-black flex items-center justify-center text-[24px] font-bold text-black bg-white">
                        {otpValue[i] || ''}
                      </div>
                    ))}
                    <input
                ...
                <button
                  ...
                  disabled={!isOtpValid}
                  className={`w-full mt-8 h-14 rounded-full flex items-center justify-center font-bold text-[18px] transition-all duration-300 ${
                    isOtpValid 
                      ? 'bg-[#FFD000] hover:bg-[#ffdb33] text-black shadow-sm transform hover:scale-[1.02]' 
                      : 'bg-[#FFD000] opacity-70 text-black cursor-not-allowed'
                  }`}
                ...
                <div className="mt-6 text-center">
                  {resendTimer !== 0 ? (
                    <p className="text-[15px] text-gray-700 font-medium">Resend OTP in {resendTimer} Sec</p>
                  ) : (
                    <button
                      type="button"
                      onClick={handleResendOtp}
                      className="text-[15px] text-black font-bold hover:text-gray-700"
                    >
                      Resend OTP
                    </button>
                  )}
        ...
        {checkoutStep === 'cart' && (
          <div className="shrink-0 w-full p-4 bg-white border-t shadow-[0_-4px_12px_rgba(0,0,0,0.06)] z-10">
              ...
              <div className="flex items-center gap-3">
                <div className="w-[25%] min-w-0 relative cursor-pointer group">
                  ...
                  <select
                    value={paymentMethod}
                    onChange={(e) => {
                      trackCleverTapEvent('Payment Method Selected', getCartEventProperties({
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (!isProcessingOrder) {
                      trackCleverTapEvent('Place Order Clicked', getCartEventProperties({
