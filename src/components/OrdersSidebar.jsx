...
export default function OrdersSidebar({ isOpen, onClose }) {
    ...
    if (isOpen) {
      const fetchOrders = async () => {
        ...
        try {
          ...
          console.log('[OrdersSidebar] fetchOrders response:', res);
          
          if (res && res.type === 'success' && res.bookings) {
            const formatted = res.bookings.map(b => ({
              id: b.bookingId || b._id.substring(b._id.length - 4),
              invoiceId: b.invoiceId || b.invoiceID || b.invoiceNo || b.invoiceNumber || b.bookingId || b._id.substring(b._id.length - 4),
              original: b,
            ...
            // Ensure they are strictly sorted from newest to oldest
            formatted.sort((a, b) => new Date(b.original.createdAt) - new Date(a.original.createdAt));
            console.log('[OrdersSidebar] Formatted orders:', formatted);
            setOrders(formatted);
          }
        } catch (error) {
  ...
  const handleBack = () => {
    console.log('[OrdersSidebar] handleBack clicked, selectedOrder:', selectedOrder?.id);
    if (selectedOrder) {
      setSelectedOrder(null);
    } else {
      onClose();
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className="fixed top-0 right-0 h-[100dvh] w-full sm:w-[400px] bg-[#f8f9fa] z-[110] flex flex-col shadow-2xl transform transition-transform duration-300 translate-x-0 font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 pb-8">
          ...
          {!selectedOrder && isLoading && (
            <div className="flex flex-col gap-4 mt-2">
              {[1, 2, 3].map((n) => (
                <div key={n} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex flex-col gap-3 animate-pulse">
          ...
          {!selectedOrder && !isLoading && orders.map((order) => (
            <div 
              ...
              onClick={() => {
                console.log('[OrdersSidebar] Order clicked:', order);
            ...
            >
              <div className="flex gap-4 items-start">
                ...
                <div className="flex flex-col">
                  ...
                  {/* <span className="text-[11px] font-bold text-gray-400 mt-0.5">INVOICE NO : {order.invoiceId}</span> */}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex flex-col">
                  ...
                  {/* <span className="font-semibold text-gray-600 text-sm mt-0.5">Invoice No : {selectedOrder.invoiceId}</span> */}
                ...
                <button 
                  onClick={async () => {
                    try {
                      ...
                      const mobileNo = original?.address?.phone || original?.address?.phoneNumber || original?.address?.mobile || original?.userId?.phone || original?.userId?.phoneNumber || original?.userId?.mobile || original?.phone || original?.phoneNumber || original?.mobile || "";
                      let profileName = "";
                      try {
                        const userStr = localStorage.getItem('petric_user');
                        if (userStr) {
                          const userObj = JSON.parse(userStr);
                          profileName = userObj?.name || "";
                        }
                      } catch (e) {}

                      const rawName = original?.address?.name || original?.userId?.name || original?.name || profileName;
                      const customerName = rawName ? rawName : (mobileNo ? `CUSTOMER [${mobileNo}]` : "CUSTOMER");

                      console.log('[OrdersSidebar] Generating invoice with rawName:', rawName, 'profileName:', profileName, 'mobileNo:', mobileNo, 'customerName:', customerName);
                      logActivity('User Clicked Download Invoice', 'Web_DownloadInvoice');

                      await generateInvoice(selectedOrder, {
                        name: customerName,
                        address: selectedOrder.deliveryDetails || original?.address?.city || original?.city || "GURGAON"
                      });
                    } catch (error) {
