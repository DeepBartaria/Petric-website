...
const BrandGrid = ({ title, brands, showName = false }) => (
  <div className="mb-14">
    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 sm:gap-6">
      {brands.map((brand, i) => (
        <Link to={`/brand/${createSlug(brand.name)}`} key={i} className="flex flex-col items-center group cursor-pointer">
...
export default function AllBrands() {
  ...
  return (
    <>
      <SEO
        title="Shop All Pet Food & Supply Brands in Gurgaon | Petric"
        description="Browse all pet supply brands available on Petric: Royal Canin, Farmina, HUFT, JerHigh, Whiskas, Pedigree & more. Fast delivery in Gurgaon."
        canonical="https://petric.in/all-brands"
      />
