...
function ProductCard({ product }) {
  return (
    <article className="bg-white border border-gray-100 rounded-2xl w-[45vw] md:w-full shrink-0 snap-center md:snap-none cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-lg flex flex-col overflow-hidden group p-2.5 md:p-4 shadow-sm">
      ...
      <div className="flex flex-col flex-grow px-0.5">
        ...
        <div className="mt-auto flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-gray-400 text-[8px] md:text-[10px] line-through">₹{product.oldPrice || '999'}</span>
            <span className="font-bold text-black text-sm md:text-base">{product.price || '₹850'}</span>
          </div>
    ...
    <div className="min-h-screen bg-white font-sans text-black">
      ...
      <main className="mx-auto max-w-[1400px] px-3 md:px-8 py-4 md:py-10">
        ...
        <div className="mb-4 text-[10px] md:text-sm font-medium text-gray-600 overflow-x-auto whitespace-nowrap">
          Home / {productData.category.split(' ; ')[0]} / {productData.brand}
        </div>

        {/* Product Section - Mobile Grid */}
        <section className="flex flex-col md:grid md:grid-cols-[1.6fr_1fr] gap-4 md:gap-6">
          {/* Product Image */}
          <div className="min-h-[240px] md:min-h-[450px] rounded-2xl md:rounded-[2rem] bg-gray-50 border border-gray-100 p-4 md:p-6 flex items-center justify-center overflow-hidden group shadow-sm">
            <img src={product1} alt={productData.name} className="max-h-[280px] md:max-h-[340px] w-auto object-contain mix-blend-multiply transition-transform duration-500 group-hover:scale-105" />
          </div>

          {/* Product Info Sidebar - Mobile Optimized */}
          <aside className="rounded-2xl md:rounded-3xl border border-gray-100 bg-white p-3 md:p-6 shadow-sm transition-all duration-300 hover:shadow-md">
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              <div className="inline-flex items-center rounded-full bg-[#FF5757] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white shadow-sm">
                Vet Recommended
              </div>
            ...
            <p className="text-xs md:text-sm font-bold text-gray-600">by: {productData.brand}</p>
            ...
            <div className="mt-3 flex items-center gap-2 text-xs md:text-sm text-gray-700">
              ...
              <span className="text-gray-500">247 orders</span>
            </div>

            {/* Size Selector */}
            <div className="mt-3">
              <p className="text-[10px] md:text-xs font-bold text-gray-500 mb-1.5">Select Size</p>
              <div className="flex gap-2">
                {productData.availableSizes.map((size) => (
                  <button
                    key={size}
                    onClick={() => setSelectedSize(size)}
                    className={`flex-1 h-9 md:h-10 px-3 rounded-lg border text-xs md:text-sm font-bold transition-all ${
                      selectedSize === size
            ...
            <div className="mt-3 flex flex-wrap items-baseline gap-3">
              <p className="text-lg md:text-2xl font-extrabold">{productData.price}</p>
              <p className="text-xs md:text-sm font-bold text-gray-400 line-through">{productData.oldPrice}</p>
              <p className="text-xs md:text-sm font-extrabold text-green-600">{productData.discount} off</p>
            </div>

            {/* Benefits Summary - Mobile Compact */}
            <div className="mt-3 space-y-1.5">
              {productData.keyBenefits.slice(0, 3).map((benefit, i) => (
                <div key={i} className="flex items-center gap-1.5 text-[10px] md:text-xs text-gray-700">
            ...
            {/* Quantity & Add to Cart - Mobile Full Width */}
            ...
            <div className="mt-3 rounded-lg bg-blue-50 border border-blue-100 p-2.5">
              <p className="text-[10px] md:text-xs font-bold text-blue-800 mb-0.5">Feeding Tip</p>
              <p className="text-[10px] md:text-xs text-blue-700 leading-snug">{productData.usageTip}</p>
            </div>
          </aside>
        </section>

        {/* H1 Headline Banner - Mobile Optimized */}
        <section className="mt-6 md:mt-8 rounded-2xl md:rounded-[2rem] bg-gradient-to-r from-[#FFF9E5] to-[#F8F9FA] border border-[#FFE880] p-4 md:p-8 shadow-sm">
          <h2 className="text-base md:text-2xl font-extrabold text-black">{productData.h1Headline}</h2>
          <p className="mt-2 text-xs md:text-sm text-gray-700 leading-relaxed">{productData.productInfo}</p>
        </section>

        {/* Product Tabs Navigation - Mobile Stacked */}
        ...
        {/* Similar Products - Mobile Grid */}
        <section className="mt-8 pb-6">
          <h2 className="mb-4 text-lg md:text-2xl font-bold text-black">Products like this...</h2>
          <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-6 overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden pb-3 pt-1 px-1 md:p-0">
