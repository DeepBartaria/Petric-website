...
import banner1 from '../assets/newb1.png';
import banner2 from '../assets/newb2.png';
import banner3 from '../assets/newb3.png';
import tempImg from '../assets/tempimg.png';
import finalHelloImg from '../assets/final_hello.png';
...
export default function NewHome() {
  ...
  const trackedProductSectionsRef = useRef(new Set());
      ...
      try {
        ...
        if (response && response.type === 'success' && response.homePageProductsData) {
          ...
          parsedSections.forEach((section) => {
            if (trackedProductSectionsRef.current.has(section.title)) return;
            trackedProductSectionsRef.current.add(section.title);
            trackCleverTapEvent('Product List Viewed', {
              'Section Name': section.title,
              'Product Count': section.products.length,
              Source: 'Home Page',
            });
          });
        }
  ...
  const trackBannerClick = (bannerName, position) => {
    trackCleverTapEvent('Banner Clicked', {
      'Banner Name': bannerName,
      Position: position,
      Source: 'Home Top Banner',
    });
    logActivity(`Click HomePageBanner ${bannerName}`, 'Click_HomePageBanner');
  };
  ...
  return (
    <>
      <SEO
        title="Pet Food & Supplies Delivered in Gurgaon in Minutes | Petric"
        description="Petric delivers dog food, cat food, pet medicines, essentials and more in Gurgaon within 59 minutes. Free delivery on all orders."
        canonical={`${window.location.origin}${window.location.pathname}`}
        image="https://petric.in/logo.png"
      />
      ...
      <main className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 md:py-10">
        ...
        <div className="hidden md:grid grid-cols-3 gap-6 mb-12">
          <div
            onClick={() => trackBannerClick('Banner 1', 1)}
          ...
          <div className="grid grid-rows-2 gap-6 h-[500px]">
            <div
              onClick={() => trackBannerClick('Banner 2', 2)}
            ...
            <div
              onClick={() => trackBannerClick('Banner 3', 3)}
              className="bg-[#D9D9D9] rounded-[2rem] w-full h-full overflow-hidden relative group cursor-pointer"
            >
        ...
        <div className="mb-14">
          <div className="flex justify-between items-end mb-6">
            ...
            <Link
              to="/all-categories"
              onClick={() => trackCleverTapEvent('Navigation Clicked', {
                'Navigation Item': 'See All Categories',
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <div className="relative flex-1 overflow-hidden">
              ...
              <div
                ...
                className="grid grid-rows-2 grid-flow-col gap-5 overflow-x-auto [&::-webkit-scrollbar]:hidden pb-4 pt-2 px-2"
              >
                {shopCategories.map((category) => (
                  <Link
                    ...
                    onClick={() => {
                      logActivity(`Click HomePageCategory ${category.name}`, 'Web_HomeCategory');
                    ...
                    className="relative shrink-0 flex flex-col items-center justify-start gap-3 w-44 cursor-pointer group"
                  >
                    <div className="w-44 h-44 rounded-full overflow-hidden bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-300 flex items-center justify-center">
                      {category.img ? (
                        <img
                          src={category.img}
                          alt={category.name}
          ...
          {/* MOBILE */}
          <div className="md:hidden overflow-x-auto [&::-webkit-scrollbar]:hidden -mx-4">
            <div className="grid grid-rows-2 grid-flow-col gap-4 px-4 pb-2 w-max">
              {shopCategories.map((category) => (
                <Link
                  key={category.id}
                  to={`/category/${createSlug(category.name)}`}
                ...
                >
                  <div className="w-32 h-32 rounded-full overflow-hidden bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-all duration-300 group-active:scale-95 flex items-center justify-center">
                    {category.img ? (
                      <img
                        src={category.img}
                        alt={category.name}
        ...
        {isHomeLoading ? (
          ...
          <div className="mb-14">
            ...
            <div className="relative">
              <div className="grid grid-rows-2 grid-flow-col md:flex md:flex-row gap-3 md:gap-5 overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden pb-5 pt-2 px-1 md:px-2">
              {topHomeSection.products.map((product, i) => (
                <ProductCard
                  ...
                  onAnimateToCart={triggerFlyToCart}
                  trackingContext={{ Section: topHomeSection?.title }}
              ...
              <button
                onClick={(e) => {
                  e.currentTarget.previousElementSibling?.scrollBy({ left: 250, behavior: 'smooth' });
                }}
                className="md:hidden absolute right-0 top-[50%] -translate-y-1/2 z-10 bg-white/95 shadow-[0_2px_10px_rgba(0,0,0,0.15)] p-2 rounded-l-full border border-gray-100 flex items-center justify-center text-black active:scale-95 transition-transform"
              >
                <FiChevronRight className="w-5 h-5" strokeWidth={3} />
              </button>
            </div>
          </div>
        )}
        ...
        <div className="mb-14">
          <div className="flex justify-between items-end mb-6">
            ...
            <Link to="/all-brands" onClick={() => trackCleverTapEvent('Navigation Clicked', {
              'Navigation Item': 'See All Brands',
              Destination: '/all-brands',
              Source: 'Home Brands',
            })} className="text-base text-black underline underline-offset-4 decoration-1 transition-all duration-300 hover:text-gray-600 hover:underline-offset-2">See all</Link>
          </div>
              ...
              >
                {brands.map((brand, i) => (
                  <Link
                    ...
                    onClick={() => {
                      logActivity(`Click HomePageBrands ${brand.name}`, 'Web_ViewBrands');
                      trackCleverTapEvent('Brand Clicked', {
        ...
        {!user && (
          <div className="mb-14 bg-gradient-to-br from-[#FFD000] via-[#FFDF33] to-[#FFF0B3] rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 flex flex-col lg:flex-row items-center justify-between gap-8 shadow-xl border border-yellow-300/40 relative overflow-hidden transition-all duration-300 hover:-translate-y-1">
            ...
            <div className="flex flex-row items-center lg:items-start justify-center lg:justify-start gap-4 md:gap-8 w-full lg:w-auto shrink-0 relative z-10">
              <div className="w-[100px] sm:w-[130px] md:w-[180px] shrink-0 overflow-hidden relative group">
                <img src={finalHelloImg} alt="Pet" className="w-full h-auto object-contain transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div className="flex flex-col items-start gap-2 sm:gap-3 md:gap-5 text-left">
                <h2 className="text-[22px] sm:text-3xl md:text-[54px] lg:text-[64px] text-black leading-[1.1] tracking-tight font-extrabold font-serif drop-shadow-sm">
                  Hello, <br className="hidden sm:block" /> Pet Parent! 👋
                </h2>
                ...
                <button
                  ...
                  className="inline-flex h-10 sm:h-12 md:h-16 lg:h-[72px] min-w-[130px] sm:min-w-[150px] md:min-w-[200px] lg:min-w-[240px] items-center justify-center rounded-full bg-white px-6 sm:px-8 md:px-12 lg:px-14 text-[13px] sm:text-[15px] md:text-[19px] lg:text-[22px] font-black text-black shadow-[0_8px_20px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(0,0,0,0.15)] hover:text-[#FF5757] ring-1 ring-black/5"
                >
                  SIGN IN NOW!
                </button>
              </div>
            ...
            <div className="flex flex-col gap-3.5 w-full max-w-sm lg:w-[340px] shrink-0 self-center lg:self-center relative z-10">
              <div className="bg-white/95 backdrop-blur-sm rounded-full px-5 py-3.5 flex items-center gap-4 shadow-[0_8px_20px_rgba(0,0,0,0.06)] border border-white/50 transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_12px_24px_rgba(0,0,0,0.1)] cursor-pointer group">
                ...
                <span className="font-bold text-gray-800 text-[14px] md:text-[15px] leading-tight">
                  Up to 30% off on <br /> selected brands
                </span>
              </div>
              ...
              <div className="bg-white/95 backdrop-blur-sm rounded-full px-5 py-3.5 flex items-center gap-4 shadow-[0_8px_20px_rgba(0,0,0,0.06)] border border-white/50 transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_12px_24px_rgba(0,0,0,0.1)] cursor-pointer group">
                ...
                <span className="font-bold text-gray-800 text-[14px] md:text-[15px] leading-tight">
                  Authentic pet supplies <br /> for you
                </span>
              </div>
              ...
              <div className="bg-white/95 backdrop-blur-sm rounded-full px-5 py-3.5 flex items-center gap-4 shadow-[0_8px_20px_rgba(0,0,0,0.06)] border border-white/50 transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_12px_24px_rgba(0,0,0,0.1)] cursor-pointer group">
                ...
                <span className="font-bold text-gray-800 text-[14px] md:text-[15px] leading-tight">
                  Emergency meds <br /> round the clock
                </span>
              </div>
            </div>
          </div>
        ...
        {isHomeLoading ? (
          ...
          <div key={idx} className="mb-14">
            ...
            <div className="relative">
              <div
                className={`grid grid-rows-2 grid-flow-col md:grid ${
                  isBestOffers
                    ? (section.products.length <= 8 ? 'md:grid-rows-none md:grid-flow-row md:grid-cols-8' : 'md:grid-flow-col md:grid-rows-2 md:auto-cols-[minmax(110px,130px)] lg:auto-cols-[minmax(120px,140px)]')
                    : (section.products.length <= 8 ? 'md:grid-rows-none md:grid-flow-row md:grid-cols-4' : 'md:grid-flow-col md:grid-rows-2 md:auto-cols-[minmax(220px,260px)] lg:auto-cols-[minmax(240px,280px)]')
              ...
              {section.products.map((product, i) => (
                <ProductCard
                  ...
                  onOpenVariants={handleOpenVariants}
                  onAnimateToCart={triggerFlyToCart}
                  trackingContext={{ Section: section.title }}
                />
              ))}
              </div>
              <button
