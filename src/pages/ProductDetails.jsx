...
export default function ProductDetails() {
  ...
  const handleAddToCart = () => {
    ...
    trackCleverTapEvent('Add To Cart Clicked', buildProductProperties(cartProduct, {
      Quantity: quantity,
      Source: 'Product Detail',
    }));
    trackCleverTapEvent('Added To Cart', buildProductProperties(cartProduct, {
      Quantity: quantity,
      'Item Value': cartProduct.discountedPrice,
      Source: 'Product Detail',
    }));
      ...
      try {
        ...
        if (res.type === 'success' && res.product) {
          ...
          if (loggedProductDetailsRef.current !== productLogId) {
            ...
            trackCleverTapEvent('Product Viewed', buildProductProperties(res.product, {
              Source: 'Product Detail',
            }));
                  ...
                  {availableSizes.map((size, i) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        trackCleverTapEvent('Variant Selected', buildProductProperties(product, {
                          'Variant ID': size._id,
                          'Variant Name': size.name || size.unit || size.weight,
                          Price: size.discountedPrice || size.originalPrice,
                          MRP: size.originalPrice,
                          Source: 'Product Detail',
                        }));
                      }}
            ...
            <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-5 overflow-x-auto md:overflow-visible snap-x snap-mandatory [&::-webkit-scrollbar]:hidden pb-3 md:pb-0">
              {brandProducts.map((p, i) => (
                <ProductCard 
                  ...
                  trackingContext={{ Recommended: 'Prod details card - Brand' }}
            ...
            <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5 overflow-x-auto md:overflow-visible snap-x snap-mandatory [&::-webkit-scrollbar]:hidden pb-3 md:pb-0">
              {similarProducts.map((p, i) => (
                <ProductCard 
                  ...
                  mobileMode="carousel" 
                  desktopMode="carousel" 
                  trackingContext={{ Recommended: 'Prod details card - Similar' }}
                  onOpenProduct={(prod) => navigate(`/product/${prod.id}`)} 
                  onOpenVariants={(prod) => navigate(`/product/${prod.id}`)}
                  onAddToCart={(prod) => {
                    const cartProduct = {
